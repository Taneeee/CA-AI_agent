...
def get_usd_inr_rate():
    """Fetch USD to INR exchange rate from multiple sources"""
...
def get_market_data(fallback_from_agent=None):
    ...
    Priority: MarketWatch -> yfinance -> GoldAPI -> Agent fallback
    """
    result = {
        "status": "error",
        "nifty_50": 0.0,
    ...
    # Try agent fallback first (if it has fresh data)
        ...
        try:
            if fallback_from_agent.get("status") == "success":
                ...
                if n and float(n) > 0:
                    ...
                    result["data_source"] = "Agent Cache"
                if g and float(g) > 0:
                ...
                if result["nifty_50"] > 0 and result["gold_price"] > 0:
    ...
    # Get sentiment and macro indicators from agent fallback
...
if generate_plan and name.strip():
    ...
    with tab1:
        ...
        with col2:
            ...
            goals = [
                ...
                f"{'🎓 Education fund' if age < 40 else '🛡️ Wealth preservation'}"
            ]
            for goal in goals:
                st.markdown(f"- {goal}")
    ...
    with tab4:
        ...
        st.subheader("⚖️ Rebalancing Guidelines")
    ...
    with tab5:
        ...
        if sentiment == 'Positive':
            ...
            st.info("⚖️ Neutral Market: Balanced approach recommended")
...
if st.session_state.get('show_profiles', False):
    ...
    if users:
        ...
        st.info("ℹ️ No saved profiles found. Generate your first plan!")
